...
BOOST_AUTO_TEST_CASE(create_server_with_empty_address_list) {
    // set up environment
    MessageBuffer msgBuffer;
    Connector *connector = new MockConnector(msgBuffer);
    ConfigBase *conf = new MockConfig();

    auto check_empty_addr_list_exception = [](const std::runtime_error &e) {
        BOOST_REQUIRE_EQUAL(e.what(), "List of IP addresses must contain at least 1 address");
        return true;
    };

    BOOST_REQUIRE_EXCEPTION(ServerTestObj server(msgBuffer, "127.0.0.1", 7777, conf, connector),
                            std::runtime_error,
                            check_empty_addr_list_exception);
}
...
BOOST_AUTO_TEST_CASE(server_handle_unexpected_message_main_loop) {
    ...
    auto check_unexpected_msg_exception = [](const std::runtime_error &e) {
        BOOST_REQUIRE_EQUAL(e.what(), "[ERROR]: Unkown state.");
        return true;
    };
